...
namespace NFramework
{
    ...
    public class UIManager : SingletonMono<UIManager>
    {
        ...
        protected void Awake()
        {
            ...
            {
                var rectTf = new GameObject(name, typeof(RectTransform), typeof(Canvas), typeof(GraphicRaycaster))
...
#if ADDRESSABLE
        public async Task<BaseUIView> OpenAddressable(string identifier, Action<BaseUIView> onOpened =
 null, bool controlInteract = true)
        {
            return await OpenAddressable<BaseUIView>(identifier, onOpened, controlInteract);
        }

        public async Task<T> OpenAddressable<T>(string identifier, Action<T> onOpened = null, bool controlInteract =
 true) where T : BaseUIView
        {
            if (controlInteract)
        ...
        public async UniTask<BaseUIView> OpenAsync(string identifier, Action<BaseUIView> onOpened = null,
            bool controlInteract = true)
        {
            return await OpenAsync<BaseUIView>(identifier, onOpened, controlInteract);
        }

        public async UniTask<T> OpenAsync<T>(string identifier, Action<T> onOpened = null, bool controlInteract = true)
            where T : BaseUIView
        {
            if (controlInteract)
                DisableInteract(this);
        ...
        public bool TryCacheView(string identifier, bool forceCacheMultiple = false)
        {
            ...
            if (!isNew && prefab.IsUnique)
                this.LogError($"UI [{identifier}] is Unique!");
        ...
        public async UniTask<bool> TryCacheViewAsync(string identifier, bool forceCacheMultiple = false)
        {
            var isNew = false;
            if (!_cachedView.ContainsKey(identifier))
            {
            ...
            {
                this.LogError($"Cannot load UI [{identifier}] from Resources", this);
                return false;
            }

            var prefab = resourceRequest.asset as BaseUIView;
            if (!isNew && prefab.IsUnique)
                this.LogError($"UI [{identifier}] is Unique!", this);
        ...
        private T OpenViewFromCached<T>(string identifier, Action<T> onOpened) where T : BaseUIView
        {
            ...
            {
                this.LogError($"Cannot push view [{identifier}] because no cached found", this);
                return null;
            }
        ...
        public bool Close(BaseUIView view, bool destroy = false)
        {
            ...
            if (index >= 0)
            {
                ...
                {
                    if (!view.CanDestroy)
                        this.LogError($"Destroy view [{view.Identifier}] with mark CanDestroy = false", this);
        ...
        public async UniTask<bool> CloseWithAnimation(string identifier, bool destroy = false)
        {
            if (IsSpecificViewShown(identifier, out var view))
            {
                await CloseWithAnimation(view, destroy);
                return true;
            }
            return false;
        }

        public async UniTask<bool> CloseWithAnimation(BaseUIView view, bool destroy = false)
        {
            var views = _openedView[view.UILayer];
            if (views.Count <= 0)
                return false;
            ...
            if (index >= 0)
            {
                await view.OnCloseWithAnimation();
                views.RemoveAt(index);
        ...
        public void EnableInteract(object register = null, bool force = false)
        {
            ...
            else if (register != null)
            {
                if (_disableInteractRegisters.Contains(register))
                    ...
                    this.LogError($"Cannot find register: {register}");
            }
